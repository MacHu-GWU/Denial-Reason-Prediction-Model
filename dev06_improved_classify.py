...
def group_category_key():
    ...
    df = df[["Revenue.Code", "Service.Code", "Procedure.Code", "Diagnosis.Code", "Subscriber.Index", 
             "Claim.Charge.Amount", "Subscriber.Payment.Amount", "Provider.Payment.Amount", "_class"]]
    df.to_csv("train_include_numeric.txt", index=False)

def classify():
    """algorithm evaluation, parameter optimization
    """
    df = pd.read_csv("train_include_numeric.txt")
    yes_dataset = df[df["_class"] == 1] # 470588
    no_dataset = df[df["_class"] == 0] # 1971

    parameter_analysis = list()
    for criterion in np.arange(0.3, 0.91, 0.05):
        ...
        for i in range(10):
            ...
            fixed_predict = list()
            for c1, c2, c3, predict in zip(
                               df.loc[test_data.index, "Claim.Charge.Amount"],
                               df.loc[test_data.index, "Subscriber.Payment.Amount"],
                               df.loc[test_data.index, "Provider.Payment.Amount"],
                               result["_predict"]):
                if predict == 0:
                    if ((c1 >= 0) and (c1 <= 105530) and (c2 != 0) and (c3 >= 0) and (c3 <= 880) ):
                        fixed_predict.append(1)
                    else:
                        fixed_predict.append(predict)
                else:
                    fixed_predict.append(predict)
            result["_predict"] = fixed_predict
    ...
    writer = pd.ExcelWriter("parameter_analysis_improved.xlsx")
    parameter_analysis.to_excel(writer, "parameter_analysis_improved", index=False)
    writer.save()
